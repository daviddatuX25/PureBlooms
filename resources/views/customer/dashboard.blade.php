        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 mb-8 border border-gray-100">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a href="{{ route('customer.dashboard') }}" class="text-rose-600 hover:text-rose-700 font-semibold text-sm">
                        View All →
                    </a>
                </div>
                ...
                <div class="flex flex-wrap gap-3">
                    <button onclick="filterProducts('all')" 
                            class="category-filter-btn px-6 py-3 rounded-full text-sm font-medium transition bg-rose-500 text-white"
                            data-category="all">
                        All Products
                    </button>
                    @foreach($categories as $category)
                    <button onclick="filterProducts('{{ $category->id }}')" 
                            class="category-filter-btn px-6 py-3 rounded-full text-sm font-medium transition bg-white text-rose-600 hover:bg-rose-100 border border-rose-200"
...
function filterProducts(categoryId) {
    ...
    document.querySelectorAll('.category-filter-btn').forEach(btn => {
        if (btn.dataset.category === categoryId) {
            btn.className = 'category-filter-btn px-6 py-3 rounded-full text-sm font-medium transition bg-rose-500 text-white';
        } else {
            btn.className = 'category-filter-btn px-6 py-3 rounded-full text-sm font-medium transition bg-white text-rose-600 hover:bg-rose-100 border border-rose-200';
        }
    });
    ...
    productCards.forEach(card => {
        if (categoryId === 'all') {
            ...
            card.style.display = productCategory == categoryId ? 'block' : 'none';
        }
    });
}
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div>
                    ...
                    <div class="space-y-2 text-sm text-gray-600">
                        <p>📱 Phone: 09667469265</p>
                        <p>📧 Email: hello@purebloom.ph</p>
                        <p>📍 Address: Bangar Cadapli</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-2 text-sm">
                        <a href="{{ route('customer.profile.show') }}" class="block text-rose-600 hover:text-rose-700">My Profile</a>
                        <a href="{{ route('customer.orders.index') }}" class="block text-rose-600 hover:text-rose-700">My Orders</a>
                        <a href="{{ route('customer.cart.index') }}" class="block text-rose-600 hover:text-rose-700">Shopping Cart</a>
                        <a href="{{ route('customer.dashboard') }}" class="block text-rose-600 hover:text-rose-700">Browse Products</a>
                    </div>
                </div>

                <!-- About -->
                <div>
                    <h4 class="text-lg font-bold text-gray-900 mb-4">🌸 About Flower Cart</h4>
                    <p class="text-sm text-gray-600 mb-4">
                        Your trusted online flower shop in the Philippines. Fresh flowers delivered with love!
                    </p>
                    <div class="flex space-x-4">
                        <span class="text-2xl cursor-pointer hover:scale-110 transition">📘</span>
                        <span class="text-2xl cursor-pointer hover:scale-110 transition">📸</span>
                        <span class="text-2xl cursor-pointer hover:scale-110 transition">🐦</span>
                    </div>
                </div>
            ...
            <div class="border-t border-gray-200 mt-8 pt-6 text-center">
                <p class="text-sm text-gray-500">
                    &copy; {{ date('Y') }} Purebloom. All rights reserved. 🌺
                </p>
            </div>
        </div>
    </footer>
